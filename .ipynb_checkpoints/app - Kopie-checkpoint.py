...
since_opts = sorted(df_orig[since_col].dropna().unique())
since_sel = st.sidebar.multiselect("Seit wann nutzt du Copilot/ChatGPT?", since_opts, default=since_opts)


freq_col = [c for c in df_orig.columns if "Wie häufig nutzt du GitHub Copilot" in c][0]
freq_opts = sorted(df_orig[freq_col].dropna().unique())
freq_sel = st.sidebar.multiselect("Wie häufig nutzt du Copilot/ChatGPT?", freq_opts, default=freq_opts)


rate_col = [c for c in df_orig.columns if "Wie bewertest du insgesamt" in c][0]
rate_opts = sorted(df_orig[rate_col].dropna().unique())
rate_sel = st.sidebar.multiselect("Gesamtbewertung", rate_opts, default=rate_opts)
...
st.title("Studie: Bewertung der Effizienz des Copilot (ChatGPT) -Einsatzes in der betrieblichen Anwendungsentwicklung")
...
fig_exp = px.bar(
    df_exp, x="Erfahrung", y="Anteil", text="Absolute",
    category_orders={"Erfahrung": order_exp},
    labels={"Absolute": "Anzahl", "Anteil": "Anteil"}
)
fig_exp.update_layout(yaxis_tickformat=".0%")
st.plotly_chart(fig_exp, use_container_width=True)


st.header("Seit wann nutzt du GitHub Copilot (ChatGPT)?")
...
fig_since = px.bar(
    df_since, x="Kategorie", y="Anteil",
    text=[f"{v:.0%}" for v in df_since["Anteil"]],
    labels={"Kategorie": clean_label(since_col), "Anteil": "Anteil"}
)
fig_since.update_layout(yaxis_tickformat=".0%")
st.plotly_chart(fig_since, use_container_width=True)


st.header("Wie häufig nutzt du GitHub Copilot (ChatGPT) in deiner Arbeit?")
...
fig_freq = px.bar(
    df_freq, x="Häufigkeit", y="Anteil",
    text=[f"{v:.0%}" for v in df_freq["Anteil"]],
    labels={"Häufigkeit": clean_label(freq_col), "Anteil": "Anteil"}
)
fig_freq.update_layout(yaxis_tickformat=".0%")
st.plotly_chart(fig_freq, use_container_width=True)


st.header("Wie bewertest du insgesamt den Einsatz von KI-Coding-Assistenten wie GitHub Copilot?")
...
fig_rate = px.bar(
    df_rate, x="Bewertung", y="Anteil",
    text=[f"{v:.0%}" for v in df_rate["Anteil"]],
    labels={"Bewertung": clean_label(rate_col), "Anteil": "Anteil"}
)
...
if task_cols:
    ...
    df_task = pd.DataFrame({
        "Aufgabe": [clean_label(c) for c in task_cols],
        "Absolute": nums.values,
        "Anteil": nums.values / len(df)
    })
    fig_task = px.bar(
        df_task, x="Aufgabe", y="Anteil", text="Absolute",
        labels={"Aufgabe": "Aufgabe", "Absolute": "Anzahl", "Anteil": "Anteil"}
    )
    fig_task.update_layout(yaxis_tickformat=".0%", xaxis_tickangle=-20)
...
for col in df.columns:
    if col.startswith("Wahrnehmung von Copilot"):
        nums = df[col].map(likert_map)
        pct  = nums.value_counts(normalize=True).sort_index()
        df_lik = pd.DataFrame({
        ...
        fig = px.bar(
            ...
            text=[f"{v:.0%}" for v in df_lik["Anteil"]],
            labels={"Antwort": "Antwort", "Anteil": "Anteil"},
            title=clean_label(col)
        )
...
if situ_col:
    st.header("In welchen Situationen ist Copilot (ChatGPT) am besonders hilfreich?")
...
fig_rel = px.bar(
    df_rel, x="Note", y="Anteil",
    text=[f"{v:.0%}" for v in df_rel["Anteil"]],
    category_orders={"Note": order_rel},
    labels={"Note": clean_label(rel_col), "Anteil": "Anteil"}
)
fig_rel.update_layout(yaxis_tickformat=".0%")
st.plotly_chart(fig_rel, use_container_width=True)


edit_col = [c for c in df.columns if "Wie oft musst du Vorschläge nachbearbeiten" in c][0]
...
fig_edit = px.bar(
    df_edit, x="Häufigkeit", y="Anteil",
    text=[f"{v:.0%}" for v in df_edit["Anteil"]],
    labels={"Häufigkeit": clean_label(edit_col), "Anteil": "Anteil"}
)
fig_edit.update_layout(yaxis_tickformat=".0%")
st.plotly_chart(fig_edit, use_container_width=True)

err_col = [c for c in df.columns if "Gab es fehlerhaften oder riskanten Code" in c][0]
...
fig_err = px.bar(
    df_err, x="Antwort", y="Anteil",
    text=[f"{v:.0%}" for v in df_err["Anteil"]],
    labels={"Antwort": clean_label(err_col), "Anteil": "Anteil"}
)
fig_err.update_layout(yaxis_tickformat=".0%")
st.plotly_chart(fig_err, use_container_width=True)

skip_col = [c for c in df.columns if "Verzichtest du bewusst" in c][0]
st.header("Verzichtest du bewusst in bestimmten Fällen auf Copilot? Wenn ja, in welchen Fällen?")
...
fig_skip = px.bar(
    df_skip, x="Antwort", y="Anteil",
    text=[f"{v:.0%}" for v in df_skip["Anteil"]],
    labels={"Antwort": clean_label(skip_col), "Anteil": "Anteil"}
)
fig_skip.update_layout(yaxis_tickformat=".0%")
...
rec_col = [c for c in df.columns if "Würdest du Copilot im professionellen Umfeld empfehlen" in c][0]
st.header("Würdest du Copilot im professionellen Umfeld empfehlen?")
...
fig_rec = px.bar(
    df_rec, x="Antwort", y="Anteil",
    text=[f"{v:.0%}" for v in df_rec["Anteil"]],
    labels={"Antwort": clean_label(rec_col), "Anteil": "Anteil"}
)
fig_rec.update_layout(yaxis_tickformat=".0%")
st.plotly_chart(fig_rec, use_container_width=True)

limit_col = next((c for c in df.columns if "Wie würdest du den Umgang einschränken" in c), None)
if limit_col:
    st.header("Wie würdest du den Umgang einschränken? z.B. keine Tests? keine Dokumentation?")
    for a in df[limit_col].dropna().unique():
        st.write("•", a)

risk_prefix = "Welche der folgenden Aspekte stellen für dich"
risk_cols = [
    c for c in df.columns
    if c.startswith(risk_prefix) and re.search(r"\([^)]+\)$", c)
]
if risk_cols:
    st.header("Welche der folgenden Aspekte stellen für dich (potenzielle) Risiken oder Hürden bei der Nutzung von GitHub Copilot/ChatGPT dar?")
    # numerisch konvertieren bevor summieren
    ...
    fig_risk = px.bar(
        ...
        x="Aspekt",
        y="Anteil",
        text=[f"{v:.0%}" for v in df_risk["Anteil"]],
        labels={"Aspekt":"Aspekt","Anteil":"Anteil der Teilnehmenden"}
    )
    fig_risk.update_layout(yaxis_tickformat=".0%", xaxis_tickangle=-20)
...
st.header("Welche Vorteile siehst du persönlich im Einsatz von GitHub Copilot/ChatGPT?")
for a in df[adv_col].dropna().unique():
    st.write("•", a)


dis_col = [c for c in df.columns if "Welche Nachteile oder Probleme" in c][0]
st.header("Welche Nachteile oder Probleme hast du bei der Nutzung von Copilot/ChatGPT erlebt?")
